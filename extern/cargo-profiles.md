...
> 本文翻译自Cargo文档，可能要很久后完成。

配置文件提供了一种改变编译器设置的方法，影响优化和调试符号。

Cargo有4中内建配置：`dev`, `release`, `test`, 与 `bench`。它根据运行的名玲，将被构建的包和目标，
以及命令行标志如`--release`自动选择配置，
选择过程如[下文所述](#profile-selection)。

配置设置可以在`Cargo.toml`中使用`[profile]`表修改，
在每个命名的配置文件中，可以用键/值对来改变个别的设置，像这样。
...
Cargo只查看工作区根部的`Cargo.toml`清单中的配置文件设置。依赖项中定义的配置设置将被忽略。

此外，配置文件可以由`config`定义覆盖。在配置文件或环境变量中指定一个配置，将覆盖`Cargo.toml`中的设置。
...
The `opt-level` setting 控制 [`-C opt-level` flag] which controls the level
